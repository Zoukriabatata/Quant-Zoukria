...
.challenge-wrap {
    ...
    border-radius: var(--r-lg); padding: 1rem 1.5rem; box-shadow: var(--shadow-card);
}
.challenge-header {
...
.bar-track { background: var(--bg-elevated); border-radius: var(--r-pill); height: 6px; overflow: hidden; }
.bar-fill  { height: 100%; border-radius: var(--r-pill); background: var(--grad-primary); transition: width .6s ease; }
.cards-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 12px; margin: .5rem 0; }
.card {
    background: var(--bg-glass); backdrop-filter: blur(12px) saturate(180%);
    border: 1px solid var(--border-default); border-radius: var(--r-lg); padding: 1.8rem;
    box-shadow: var(--shadow-card); background-image: var(--grad-surface);
    transition: var(--t-normal); position: relative; overflow: hidden;
}
.card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; border-radius: var(--r-lg) var(--r-lg) 0 0; }
...
.card-num { font-family: 'JetBrains Mono',monospace; font-size: .62rem; color: var(--text-muted); letter-spacing: .12em; margin-bottom: 1rem; opacity: .5; }
...
.card-tag { display: inline-flex; align-items: center; margin-top: 1.2rem; padding: .25rem .75rem; border-radius: var(--r-pill); font-size: .6rem; font-weight: 600; letter-spacing: .08em; font-family: 'JetBrains Mono',monospace; border: 1px solid transparent; }
...
.arch { background: var(--bg-surface); border: 1px solid var(--border-default); border-radius: var(--r-lg); padding: 1.4rem 1.8rem; font-family: 'JetBrains Mono',monospace; font-size: .8rem; line-height: 2.1; margin-top: .5rem; box-shadow: var(--shadow-card); }
...
.dd-wrap { background: var(--bg-surface); border: 1px solid var(--border-default); border-radius: var(--r-lg); padding: 1rem 1.5rem; margin-top: 1rem; box-shadow: var(--shadow-card); }
.dd-header { display: flex; justify-content: space-between; font-family: 'JetBrains Mono',monospace; font-size: .68rem; color: var(--text-muted); margin-bottom: .5rem; }
...
.footer { text-align: center; padding: 2rem; color: var(--border-default); font-size: .7rem; font-family: 'JetBrains Mono',monospace; letter-spacing: .12em; }
</style>
...
<div class="hero anim-fade-up">
    <div class="hero-tag">
        ...
        HURST MR · MNQ FUTURES · APEX TRADER FUNDING 50K
    </div>
    <h1 class="hero-title">QUANT<br><span>MATHS</span></h1>
    <p class="hero-sub">Hurst_MR · K=3.00σ · Walk-Forward Validated · Apex $50K EOD Challenge</p>
    <div class="challenge-wrap">
        <div class="challenge-header">
            <span>CHALLENGE PROGRESS</span>
            <span class="challenge-pct">{pnl_sign}{s['pnl']:.0f}$ / {CHALLENGE_TARGET:.0f}$ &nbsp;·&nbsp; {prog_pct}%</span>
        </div>
        <div class="bar-track"><div class="bar-fill" style="width:{min(prog_pct,100)}%"></div></div>
    </div>
</div>
""", unsafe_allow_html=True)
...
_cu_stats = [
    {"value": round(s["wr"], 1) if s["n"] > 0 else 0, "label": "Win Rate", "suffix": "%", "decimals": 1, "color": _wr_color, "sub": f"LIVE · {s['n']} TRADES" if s["n"] > 0 else "EN ATTENTE", "static": s["n"] == 0},
    {"value": round(s["rr"], 2) if s["n"] > 0 else 0, "label": "Risk / Reward", "suffix": "x", "decimals": 2, "color": _rr_color, "sub": "AVG WIN / AVG LOSS", "static": s["n"] == 0},
    {"value": abs(round(s["pnl"])), "label": "P&L Challenge", "prefix": ("+" if s["pnl"] >= 0 else "-"), "suffix": "$", "decimals": 0, "color": _pnl_color, "sub": f"TARGET {CHALLENGE_TARGET:.0f}$"},
    {"value": round(s["dd_rem"]), "label": "DD Restant", "suffix": "$", "decimals": 0, "color": _dd_color, "sub": f"MAX {CHALLENGE_DD:.0f}$"},
    {"value": s["n"], "label": "Trades", "decimals": 0, "color": "#f1f5f9", "sub": "CHALLENGE TOTAL"},
]
...
<div class="dd-wrap">
    ...
    <div class="bar-track-dd"><div class="bar-fill-dd" style="width:{dd_pct_used}%"></div></div>
</div>
""", unsafe_allow_html=True)
...
<div class="cards-grid">
    <div class="card card-1 anim-fade-up anim-stagger-1">
        ...
        <div class="card-desc">Roadmap structurée — fBm, Hurst R/S,<br>HMM, GARCH, Z-score.<br>Fiches · Formules · Visualisations.</div>
    ...
    <div class="card card-2 anim-fade-up anim-stagger-2">
        ...
        <div class="card-desc">5 ans MNQ M1 Databento.<br>Walk-forward · PF 2.03 · Sharpe 2.50.<br>Validé hors-sample sur 8 fenêtres OOS.</div>
    ...
    <div class="card card-3 anim-fade-up anim-stagger-3">
        ...
        <div class="card-desc">Ticks temps réel via Rithmic · NinjaTrader.<br>Hurst rolling · Z-score · Discord push.<br>Journal intégré · Stats challenge live.</div>
        <span class="card-tag tag-cyan">RITHMIC · TEMPS RÉEL</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="arch">
    <div class="arch-row"><span class="arch-key">SIGNAL</span><span class="arch-arrow">→</span><span class="arch-val">Hurst R/S rolling &lt; 0.53 + |Z-score| &gt; 3.00σ → Mean Reversion</span></div>
    ...
    <div class="arch-row"><span class="arch-key">DATA</span><span class="arch-arrow">→</span><span class="arch-val">Rithmic · NinjaTrader RithmicBridge → C:/tmp/mnq_live.json → Streamlit</span></div>
    ...
    <div class="arch-row"><span class="arch-key">CHALLENGE</span><span class="arch-arrow">→</span><span class="arch-val">Apex Trader Funding 50K EOD · DD max 2 000$ · Target 3 000$</span></div>
</div>
""", unsafe_allow_html=True)
